...
const allowedOrigins = ["http://192.168.1.18:8080","http://127.0.0.1:8000", "http://127.0.0.1:8080", "https://quotidie.netlify.app", "https://dnd-app.netlify.app"];
...
async function get_rencontre(req, resp) {
    ...
    ///monsterAPI/get_rencontre?FPmin=1/2&FPmax=9&alignement=AA&types=&noms=Aboleth,Allosaure&number=3&ident=false
    // CORS STUFF
    ...
    let noms = decodeURI(req.query.noms) == "undefined" ? [] : decodeURI(req.query.noms);
    noms = noms.split(",");
    if (noms[0] === "") {
        noms = [];
    }
    ...
    console.log(FPmin, FPmax, number, alignement, ident, types, noms);
            ...
            if (eval(monster.FP) >= FPmin) {
                if (alignement[0] == monster.alignement[0] || alignement[0] == "A") {
                    if (alignement[1] == monster.alignement[1] || alignement[1] == "A") {
                        if (types.includes(monster.type) || types.length == 0) {
                            if (noms.includes(monster.nom) || noms.length == 0) {
